...
ganados = 0
veces_calculo_probabilidades = 0
movimientos = 0
...
class WumpusAgent:
  ...
  def choose_actions(self, environment):
    ...
    if cell.breeze:
      ...
      global veces_calculo_probabilidades 
      veces_calculo_probabilidades += 1
      
      environment.update_probabilities_frontier()
      frontier = environment.get_probability_frontier()

      probabilities = []
      print("Celdas opcionadas a visitar :")
...
class WumpusEnvironment:
  ...
  def execute_actions(self, actions, agent):
    while actions:
      ...
      if tuple(self.agent_location) == tuple(self.wumpus_location):
...
def main():
  ...
  num_calculos_probs_exp= []
  num_moves_exp = []
  grid_size = int(input("Ingrese el tamaño de la grilla (cuadrada): "))
  experiments = int(input("Ingrese numero de experimentos a realizar: "))
  ...
  for i in range(experiments):
    ...
    while True:  # Bucle infinito
      ...
      if end_game[0]: 
        ganados += end_game[1]
        num_calculos_probs_exp.append(veces_calculo_probabilidades)
        veces_calculo_probabilidades = 0
        num_moves_exp.append(movimientos)
        movimientos = 0
        break

  print()
  print()
  print(f"FIN DE LOS EXPERIMENTOS")
  print()
  print(f"Ganados: {ganados}")
  print(f"Perdidos: {experiments - ganados}")
  print(f"Promedio de veces que se calcularon probabilidades: {np.mean(num_calculos_probs_exp)}" )
  print(f"Desviación estandar de veces que se calcularon probabilidades: {np.std(num_calculos_probs_exp)}")

  print(f"Promedio de movimientos: {np.mean(num_moves_exp)}" )
  print(f"Desviación estandar de movimienots: {np.std(num_moves_exp)}")
